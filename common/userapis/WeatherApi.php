...
class WeatherApi
{
    ...
    {
        $ret = preg_match('/(.+)天气/i', $message, $matchs);
        if(!$ret)
        {
        ...
        if (isset($obj->Weather->city))
        {
            ...
            $response = $data->city . '今日天气' . PHP_EOL .
                ...
                '==================' . PHP_EOL .
                '【穿衣指数】：' . $data->chy_shuoming . PHP_EOL .PHP_EOL .
                '【感冒指数】：' . $data->gm_l . $data->gm_s . PHP_EOL .PHP_EOL .
                '【空调指数】：' . $data->ktk_s . PHP_EOL .PHP_EOL .
                '【污染物扩散条件】：' . $data->pollution_l . $data->pollution_s . PHP_EOL .PHP_EOL .
                '【洗车指数】：' . $data->xcz_l . $data->xcz_s . PHP_EOL .PHP_EOL .
                '【运动指数】：' . $data->yd_l . $data->yd_s . PHP_EOL .PHP_EOL .
                '【紫外线指数】：' . $data->zwx_l . $data->zwx_s . PHP_EOL .PHP_EOL .
