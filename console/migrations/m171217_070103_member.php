<?php

use yii\db\Migration;

class m171217_070103_member extends Migration
{
    public function up()
    {
        /* 取消外键约束 */
        $this->execute('SET foreign_key_checks = 0');
        
        /* 创建表 */
        $this->createTable('{{%member}}', [
            'id' => 'int(11) NOT NULL AUTO_INCREMENT COMMENT \'主键\'',
            'username' => 'varchar(20) NULL DEFAULT \'\' COMMENT \'帐号\'',
            'password_hash' => 'varchar(255) NULL DEFAULT \'\' COMMENT \'密码\'',
            'auth_key' => 'varchar(32) NULL DEFAULT \'\' COMMENT \'授权令牌\'',
            'password_reset_token' => 'varchar(255) NULL DEFAULT \'\' COMMENT \'密码重置令牌\'',
            'wechat_openid' => 'varchar(50) NULL DEFAULT \'\' COMMENT \'微信openid\'',
            'type' => 'tinyint(1) NULL DEFAULT \'1\' COMMENT \'类别[1:普通会员;10管理员]\'',
            'nickname' => 'varchar(50) NULL DEFAULT \'\' COMMENT \'昵称\'',
            'realname' => 'varchar(10) NULL DEFAULT \'\' COMMENT \'真实姓名\'',
            'head_portrait' => 'varchar(255) NULL DEFAULT \'\' COMMENT \'头像\'',
            'sex' => 'tinyint(1) NULL DEFAULT \'1\' COMMENT \'性别[1:男;2:女]\'',
            'qq' => 'varchar(20) NULL DEFAULT \'\' COMMENT \'qq\'',
            'email' => 'varchar(60) NULL DEFAULT \'\' COMMENT \'邮箱\'',
            'birthday' => 'date NULL COMMENT \'生日\'',
            'user_money' => 'decimal(10,2) NULL DEFAULT \'0.00\' COMMENT \'余额\'',
            'accumulate_money' => 'decimal(10,2) NULL DEFAULT \'0.00\' COMMENT \'累积消费\'',
            'frozen_money' => 'decimal(10,2) NULL DEFAULT \'0.00\' COMMENT \'累积金额\'',
            'user_integral' => 'int(11) NULL DEFAULT \'0\' COMMENT \'当前积分\'',
            'address_id' => 'mediumint(8) unsigned NULL COMMENT \'默认地址\'',
            'visit_count' => 'smallint(5) unsigned NULL DEFAULT \'0\' COMMENT \'访问次数\'',
            'home_phone' => 'varchar(20) NULL DEFAULT \'\' COMMENT \'家庭号码\'',
            'mobile_phone' => 'varchar(20) NULL DEFAULT \'\' COMMENT \'手机号码\'',
            'passwd_question' => 'varchar(50) NULL DEFAULT \'\' COMMENT \'密码问题\'',
            'passwd_answer' => 'varchar(255) NULL DEFAULT \'\' COMMENT \'密码答案\'',
            'role' => 'smallint(6) NULL DEFAULT \'10\' COMMENT \'权限\'',
            'status' => 'smallint(6) NULL DEFAULT \'10\' COMMENT \'状态\'',
            'last_time' => 'int(10) NULL DEFAULT \'0\' COMMENT \'最后一次登陆时间\'',
            'last_ip' => 'varchar(16) NULL DEFAULT \'\' COMMENT \'最后一次登陆ip\'',
            'country' => 'varchar(100) NULL DEFAULT \'\' COMMENT \'国家\'',
            'province' => 'varchar(100) NULL DEFAULT \'\' COMMENT \'省\'',
            'city' => 'varchar(100) NULL DEFAULT \'\' COMMENT \'市\'',
            'created_at' => 'int(10) NOT NULL DEFAULT \'0\' COMMENT \'创建时间\'',
            'updated_at' => 'int(10) NULL DEFAULT \'0\' COMMENT \'修改时间\'',
            'PRIMARY KEY (`id`)'
        ], "ENGINE=InnoDB  DEFAULT CHARSET=utf8mb4 COMMENT='用户_会员表'");
        
        /* 索引设置 */
        
        
        /* 表数据 */
        $this->insert('{{%member}}',['id'=>'1','username'=>'admin','password_hash'=>'$2y$13$sra9VTJBdCU055l8Q0ZFzOtL0hOqzjq9fWJYL6xTa1noYbCVHmBAq','auth_key'=>'vHMbVgWt_s4qaENeDaPyThdBSgFT--Rc','password_reset_token'=>NULL,'wechat_openid'=>NULL,'type'=>'1','nickname'=>NULL,'realname'=>NULL,'head_portrait'=>NULL,'sex'=>'1','qq'=>NULL,'email'=>'1@qq.com','birthday'=>NULL,'user_money'=>'0.00','accumulate_money'=>'0.00','frozen_money'=>'0.00','user_integral'=>'0','address_id'=>'0','visit_count'=>'1','home_phone'=>NULL,'mobile_phone'=>NULL,'passwd_question'=>NULL,'passwd_answer'=>NULL,'role'=>'10','status'=>'10','last_time'=>'1511169880','last_ip'=>'127.0.0.1','country'=>NULL,'province'=>NULL,'city'=>NULL,'created_at'=>'1511169880','updated_at'=>'1511169880']);
        
        /* 设置外键约束 */
        $this->execute('SET foreign_key_checks = 1;');
    }

    public function down()
    {
        $this->execute('SET foreign_key_checks = 0');
        /* 删除表 */
        $this->dropTable('{{%member}}');
        $this->execute('SET foreign_key_checks = 1;');
    }
}

