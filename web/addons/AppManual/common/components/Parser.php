<?php
namespace addons\AppManual\common\components;
    ...
    private function makeFootnotes($html)
    {
        if (count($this->_footnotes) > 0) {
            ...
            while ($val = array_shift($this->_footnotes)) {
                if (is_string($val)) {
                    $val .= " <a href=\"#fnref-{$index}\" class=\"footnote-backref\" target=\"_blank\">&#8617;</a>";
                } else {
                    $val[count($val) - 1] .= " <a href=\"#fnref-{$index}\" class=\"footnote-backref\" target=\"_blank\">&#8617;</a>";
    ...
    public function parseInline($text, $whiteList = '', $clearHolders = true, $enableAutoLink = true)
    {
        ...
        $text = preg_replace_callback(
            ...
            function ($matches) use ($self) {
                ...
                return $self->makeHolder(
                    "<sup id=\"fnref-{$id}\"><a href=\"#fn-{$id}\" class=\"footnote-ref\" target=\"_blank\">{$id}</a></sup>"
                );
            },
            $text
        );
        ...
        $text = preg_replace_callback(
            ...
            function ($matches) use ($self) {
                ...
                return $self->makeHolder(
                    "<img src=\"{$url}\" alt=\"{$escaped}\" title=\"{$escaped}\" width='100%'>"
                );
            },
            $text
        );
        ...
        $text = preg_replace_callback(
            ...
            function ($matches) use ($self) {
                ...
                $result = isset( $self->_definitions[$matches[2]] ) ?
                    "<img src=\"{$self->_definitions[$matches[2]]}\" alt=\"{$escaped}\" title=\"{$escaped}\" width='100%'>"
                    : $escaped;
